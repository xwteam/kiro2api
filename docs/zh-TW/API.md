...
> **v0.16.0:管理端兩處行為變化**
> - **改優先級立即生效**。此前黏滯選號檔下改了優先級不觸發重新選號,等於沒改。
> - **全池被判停後會自癒**。上游抖一陣把所有帳號判停時,池子不再就此徹底不可用到有人重啟;
>   自癒**不會**喚醒被永久停用、被封禁、或配額尚未到恢復時刻的帳號。
> **v0.15.0:兩處行為變化**
> - **額度耗盡按恢復時刻落盤**(憑證新增 `quotaResetUnix`)。此前只置記憶體態,每次重啟就
>   忘光、再拿使用者的請求去重新發現(前幾次會失敗)。現在重啟仍記得,**到點自動回池**,
>   不必手動重設。恢復時刻優先取餘額介面的 `nextResetAt`。管理面「重設」會清掉該標記。
> - **伺服器端內建搜尋 `web_search` 已真正實作**。只宣告這一個工具時,本服務會攔下請求、
>   呼叫上游 MCP 端點,並回傳 `server_tool_use` + `web_search_tool_result` 兩個內容區塊。
>   混著其它工具時不接管(那種請求由模型決定呼叫哪個)。搜尋失敗回傳空結果集而非 5xx。
...
> 同一帳號的**資料面、權杖重新整理、餘額查詢、模型清單、背景續期一律走同一個出口**。
> **v0.10.0 起,被判不可用的帳號一律不再自行回池。** 此前只有 `banned` 是這樣;額度耗盡(402)
> 走 30 分鐘冷卻、連續兩次無明確失效訊號的 401/403 走 5 分鐘冷卻,到點都會自動回到輪轉裡。
> 後果是一個**已被上游封禁**的帳號每 5 分鐘被重新拿去打一次、永不停止,一個額度耗盡的帳號
> 每 30 分鐘去撞一次必然失敗的牆(一天 48 次)。現在這兩類與 `banned` 一樣停止使用。
> 與 `banned` 的差別在於**持久性**:它們只置記憶體態,重啟或「重設」即復活;`banned` 則隨
> `statusReason` 落盤。429 不在此列——它已改判為瞬時限流(退避重試,不罰帳號)。
...
| 502 | 上游拉取失敗 | `{"success":false,"id":"…","error":"models upstream HTTP 403: …"}`。**每個上游請求都帶 `Connection: close`、且客戶端不複用連線**:本中轉每個請求攜帶**不同帳號**的令牌,user-agent 裡的 machineId 也隨帳號變化,複用連線會讓同一條 TCP/TLS 上依次出現數十個不同身分 —— 真實客戶端不可能如此。瞬態失敗換帳號前指數退避 200ms→2s + 抖動;帳號級失敗不退避上游連線鎖 **HTTP/1.1**(真實客戶端不主動升 h2;且 `Connection: close` 是 1.1 的標頭,h2 明確禁止它),TLS 後端預設 **native-tls(OpenSSL)** 以貼合真實客戶端的 ClientHello 指紋 —— 該指紋在任何 HTTP 內容發出**之前**就暴露。 |
