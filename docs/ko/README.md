...
| 날짜 | 업데이트 내용 |
|------|----------|
| 2026-08-19 | v0.17.2 - 📄 **문서만 수정. 코드는 v0.17.1과 완전히 동일하며 재배포가 필요 없습니다**. ①다국어 README 갱신 이력 중 9행이 잘못된 언어였습니다(번체에 간체, 일본어와 한국어에 영어). 두 가지 문안을 여섯 파일에 돌려 쓰고 언어별로 작성하지 않은 것이 원인입니다. 현재 60행 전체를 한 줄씩 확인했습니다. ②5개 API.md에 버전 계약이 빠져 있었습니다 — v0.15.0은 일본어·한국어·번체 세 곳에 아예 없었고, 그 버전이 바로 서버 내장 검색이 실제로 연결된 버전(`server_tool_use` / `web_search_tool_result` 추가)이라 이 세 언어 독자는 전혀 볼 수 없었습니다. v0.16.0은 다섯 곳 모두 누락. 현재는 모두 갖춰졌습니다. ③v0.17 계약 문안을 추가하고 남은 옛 버전 번호를 정리했습니다 |
| 2026-08-11 | v0.17.1 - 🩹 **v0.17.0에서 들어온 회귀 수정**: 세 프로토콜(OpenAI/Gemini/Responses)의 스트리밍 출구가 **응답 끝부분을 삼킬** 수 있었습니다. 지난 버전에서 thinking 분리를 연결할 때 스플리터의 "투입"만 연결하고 "스트림 종료 시 비우기"를 연결하지 않았습니다 — 스플리터는 청크에 걸쳐 잘린 `<thinking`을 잡기 위해 마지막 몇 바이트를 보류하고 종료 시 flush로 내보냅니다. 이것이 없으면 응답 끝에 `<` 하나만 나와도(코드의 `<div`, `<Component`는 매우 흔함) 그 부분이 영영 사라집니다. 네이티브 Anthropic 출구는 처음부터 정상이었고, 이번에 새로 연결한 셋에서 누락되었습니다 |
| 2026-08-10 | v0.17.0 - 🎯 **지난 모든 수정에 같은 질문을: 정말 모든 호출 지점에 도달했는가?** 공용 함수를 고쳐도 어떤 호출부는 그대로거나, 같은 이름의 비공개 함수가 공용 구현을 가려 전체 검색에서는 "통일됨"처럼 보인다. 이번에 26곳을 찾아 고쳤고, 재점검에서 절반만 된 6곳이 더 나와 함께 처리했다. 주요 항목: ①**모든 ksk 계정이 데이터 플레인에서 같은 machineId를 공유** — 비공개 구현이 refresh_token에서 파생하는데 ksk에는 그것이 없어 전부 같은 상수로 수렴(한 대가 수십 계정을 돌리는 형태), 동시에 잔액/모델 목록은 각자 계산; ②**200이어도 이벤트 스트림에 exception이 있으면 성공으로 기록** — 200을 받는 순간 성패가 확정되어 불량 계정은 결코 강등되지 않고 재시도도 없음; ③**구 활성화/비활성화 API는 저장하지 않고, 정상 종료(docker stop)는 풀을 전혀 플러시하지 않음** — 컨테이너가 재시작될 때마다 한도/차단/machineId 판정을 잊고 사용자 한도로 다시 학습; ④토큰 연장이 별도의 가용성 판정을 써서 "한도 소진" 단계가 빠져, 이미 거부된 계정을 계속 갱신; ⑤**축약된 도구 이름 복원이 4개 스트리밍 출구 중 1개에만 있어**, 나머지 3개 프로토콜은 클라이언트가 선언하지 않은 도구 이름을 받음. 추가: OpenAI/Gemini/Responses의 extended thinking(응답 측 thinking 분리도 함께 — 켜기만 하고 분리하지 않으면 본문에 섞여 더 나쁨), 이 셋에 세션 식별자, Gemini 스트림 SSE 킵얼라이브, CORS, `KIRO_API_KEY` 환경 변수 |
| 2026-08-10 | v0.16.0 - 🔍 **독립 재점검을 한 번 더 돌려 6곳을 발견 — 그중 몇 곳은 과거 수정이 절반만 반영된 것**. ①`amz-sdk-invocation-id`의 UUID 수정이 **5개 호출 지점 중 3곳에만 반영**: balance와 models_cache가 같은 이름의 옛 생성기를 각자 비공개로 두어 공용 구현을 가렸고, 전체 검색에서는 "통일됨"처럼 보였습니다. 실제로는 같은 Bearer가 데이터 플레인에서는 UUID를, 잔액 조회에서는 맨 hex를 보냈습니다; ②**로그인 흐름이 oidc 호스트로 헤더를 하나도 보내지 않고 UA조차 없음** — 갱신 체인에서 이미 고쳤던 것과 같은 결함이며, 로그인은 **같은 호스트의 인접 경로**를 칩니다. 등록 때는 맨몸, 몇 분 뒤 같은 clientId가 완벽한 SDK 헤더로 갱신하는 이 모순은 맨 요청보다 더 쉽게 연관됩니다; ③**region을 세 체인이 각자 계산**(데이터 플레인은 profileArn, 잔액/모델은 맨 cred.region) → 잔액을 영영 조회 못 하고, 같은 Bearer가 두 region에 동시에 적중; ④history의 도구 이름이 축약을 거치지 않아 tools 목록의 축약명과 불일치; ⑤**우선순위를 바꿔도 재선택이 일어나지 않음**(스티키에서는 바꾼 의미가 없음); ⑥**전체 풀 정지 후 자가 복구가 없음** — 상류가 잠시 흔들려 전부 정지되면 누군가 재시작할 때까지 풀은 완전히 사용 불능. 그 외: README 갱신 이력은 최근 10건만, 전체는 CHANGELOG 참조 |
| 2026-08-10 | v0.15.0 - 🔁 **한도가 소진된 계정을 재시작할 때마다 다시 학습하지 않습니다**(사용자가 직접 물었습니다: "한도를 다 쓴 계정은 이미 비활성화된 것 아닌가요? 왜 비활성 계정으로 요청이 가죠?"). 비활성화는 되어 있었지만 **메모리에만** 있었습니다 — 이전에 한 번의 흔들림으로 계정을 영구히 죽이지 않으려고 런타임 정지는 저장하지 않기로 했는데, 한도 소진은 바로 **명확한 복구 시각이 있는** 종류였습니다. 그래서 릴리스마다 재시작하며 "누가 한도 소진인지"를 잊고 **사용자의 요청**으로 다시 발견했습니다(실측: 소진 13건에서 처음 2번이 502, 3번째에야 성공) → 이제 복구 시각과 함께 저장하여 재시작 후에도 기억하고, 시각이 되면 자동으로 풀에 복귀합니다. 복구 시각은 상류의 `nextResetAt`을 우선하고 없으면 다음 달 1일로 추정합니다. 그 외: **서버 내장 검색 `web_search`가 실제로 연결되었습니다** — 이전에는 이 도구 선언을 "허용"만 했을 뿐(400을 내지 않음) 실제로 검색하지 않았고, 모델이 평범한 문장을 돌려주면 클라이언트는 검색했다고 믿었습니다. 이제 이런 요청은 데이터 플레인에 들어가기 전에 가로채 상류 `/mcp` 결과로 `server_tool_use` + `web_search_tool_result`를 합성합니다 |
| 2026-08-10 | v0.14.1 - 🔧 **운영 검증 중 즉석에서 발견한 두 가지**. ①응답의 `usage.input_tokens`가 **항상 0**: 이전에 추가한 입력 추정이 과금으로만 전달되고 클라이언트로 되돌려 쓰이지 않았습니다 — 청구서에는 있는데 응답에는 없고, 클라이언트는 이 값으로 비용과 컨텍스트 사용량을 계산합니다 → 비스트리밍 `usage`와 스트리밍 `message_start` 모두 같은 값을 담습니다; ②**소진된 계정이 풀에 쌓여 있으면 사용자의 처음 몇 번이 연속 실패**: 한도 소진은 그동안 인증 실패와 3회 재시도 예산을 공유했고, 실측에서는 소진 13건에 대해 502를 두 번 태우고 세 번째에 성공했습니다 → 한도 소진은 **결정적** 결론(이번 주기에는 누구로 바꿔도 같음)이므로 "모델 사용 불가"와 같은 계정 단위 결정적 등급으로 옮기고 예산은 풀 크기에 따라 부여합니다. 일시적/인증 계열은 종전대로 3회의 작은 상한을 유지합니다. 풀 전체가 정말 소진된 경우에는 `429`로 한도 문제임을 분명히 밝히며, 모호한 502를 내지 않습니다 |
...
| `KIRO_API_KEY` | ❌ | 없음 | Kiro API 키(`ksk_` 접두사) 하나만으로 서비스를 기동합니다: 시작 시 계정 풀에 병합되어 그대로 저장되며, 동일한 키는 중복 가져오지 않습니다. 마운트 볼륨에 자격 증명 파일이 없어도 이것만으로 실행됩니다 |
