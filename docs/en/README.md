...
| Date | Update |
|------|--------|
| 2026-08-19 | v0.17.2 - 📄 **Documentation only — the code is identical to v0.17.1, no redeploy needed.** ①Nine rows in the multilingual README update tables were in the wrong language (Traditional Chinese got Simplified; Japanese and Korean got English), because two texts were reused across six files instead of writing each language; all 60 rows have now been checked individually. ②Version contracts were missing from the five API.md files — v0.15.0 was absent from the Japanese, Korean and Traditional Chinese copies, and that is the release where server-side search actually landed (adding `server_tool_use` / `web_search_tool_result`), so readers of those three languages could not see it at all; v0.16.0 was missing from all five. Both are now complete. ③Added the v0.17 contract text and swept stale version strings |
| 2026-08-11 | v0.17.1 - 🩹 **Fixes a regression from v0.17.0**: the OpenAI/Gemini/Responses streaming exits could **swallow the tail of a response**. When thinking-stripping was wired into them last release, only the splitter's feed side was connected, not the end-of-stream flush — the splitter holds back the last few bytes to catch a `<thinking` tag split across chunks, so without that flush any response ending in a `<` (`<div`, `<Component` — very common in code) lost its tail. The native Anthropic exit was always correct; these three were missed when newly wired |
| 2026-08-10 | v0.17.0 - 🎯 **One question, asked of every past fix: did it actually reach every call site?** A shared helper gets fixed but one caller never switches to it — or a file keeps a **private function of the same name** that shadows the shared one, so a repo-wide search makes it look unified. This round found 26 such gaps; a re-audit after fixing them caught 6 more that were only half-done. Highlights: ①**every ksk account shared one machineId on the data plane** — the private copy derived it from the refresh token, which ksk credentials simply don't have, so they all collapsed onto one constant (one machine appearing to run dozens of accounts), while balance/model-list paths computed their own; ②**a 200 carrying an exception inside the event stream was recorded as success** — the verdict was written the moment the 200 arrived, so bad accounts were never penalised and never retried elsewhere; ③**the legacy enable/disable endpoints never persisted, and normal shutdown (docker stop) never flushed the pool** — every container restart forgot quota/ban/machineId verdicts and relearned them at your expense; ④token keep-alive used a second availability rule missing the quota tier, so already-rejected accounts kept getting refreshed; ⑤**shortened tool names were restored on only one of four streaming exits**, leaving three protocols to hand clients a tool name they never declared. Added: extended thinking on OpenAI/Gemini/Responses (with thinking stripped on the way back — enabling it without that is worse than leaving it off), session identity on those three, Gemini SSE keep-alive, CORS, and a `KIRO_API_KEY` env var |
| 2026-08-10 | v0.16.0 - 🔍 **Another independent re-audit turned up six gaps — several of them earlier fixes that had only landed halfway.** ① The `amz-sdk-invocation-id` UUID fix had reached **3 of 5 call sites**: balance and models_cache each kept a private function of the same name, shadowing the shared one, so a repo-wide search made it look unified while the same Bearer sent a UUID on the data plane and bare hex on the balance query. ② **The login flow sent no headers at all to the oidc host — not even a User-Agent** — the very defect fixed for token refresh in v0.10.0, on an adjacent path of the *same host*; worse, registration went bare while the same `clientId` refreshed minutes later with perfect aws-sdk-js headers, and that inconsistency correlates more easily than plain bareness. ③ **Region resolution differed across three links** (data plane used the profileArn, balance and model list used a bare `cred.region`), so balance queries never resolved and one Bearer hit two regions within a minute. ④ Tool names in the history skipped the shortening applied to the tool list. ⑤ **Changing priority did not trigger reselection**, making it a no-op in the sticky default. ⑥ **A fully stopped pool had no path back** — an upstream wobble could stop every account and leave the pool dead until someone restarted it. Also: the README history now keeps only the 10 most recent entries |
...
| `KIRO_API_KEY` | ❌ | none | Start the service straight from a single Kiro API Key (`ksk_` prefix): it is merged into the account pool at startup and persisted; an identical key is not imported twice. Enough to run with no credentials file in the mounted volume |
