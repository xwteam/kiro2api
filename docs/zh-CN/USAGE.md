...
打开仪表盘时面板会**静默自动检查更新**。当 GitHub 上存在更新版本时，「检查更新」按钮会高亮为「更新到 vX」。点击后弹出「更新服务 vX」弹窗，在可滚动的框里展示**当前界面语言**对应的版本更新说明，并附上升级命令 `docker compose pull && docker compose up -d` 与一键复制按钮。弹窗只做提示/展示，**不会自动执行升级**。
...
也可以选择批量导入：每行一个 bearer / SSO token，或粘贴凭据数组 / `{accounts}` 对象。导入时 kiro2api 会**逐条添加**：每加一个账号，立即对它查一次余额（真实调用上游 `getUsageLimits`）以**验证账号存活**——存活的保留，失活的自动回滚删除并过滤掉。同时按 refresh token **去重**：账号池里已存在的直接跳过，绝不重复导入同一账号（否则两份凭据会争抢同一个轮换令牌，导致相互失效、白白消耗配额、触发上游风控）。

导入弹窗会**实时展示逐条进度**：一条进度条 +「正在处理第 i/N 个账号」提示、成功 / 重复 / 失败的实时计数，以及一份逐账号状态列表——每行随处理进程实时更新（待处理 → 校验中 → 验证中 → 已验证并附该账号用量 / 重复 / 失败已排除）。验证通过的账号会**即时保存**，中途打断也能保留已成功的部分；导入过程中弹窗**无法关闭**。
...
> [!NOTE]
> 日志功能需 `logCapacity > 0`（默认 `5000`）。设为 `0` 时关闭日志捕获，日志端点返回 503。
...
向任意可用模型发一条测试请求，直连中转并展示原始返回结果，便于验证某个账号/模型是否真正可用。

- 选择模型（可选指定端点），点击发送即可
- 请求用你已创建的某个 API-KEY 调用中转端点；**当尚未创建任何自定义 key 时，默认使用主 API-KEY（`adminApiKey` / `apiKey`）**，开箱即可测试
- 该 key 仅保存在浏览器本地（localStorage），只用于调用中转端点
...
> [!NOTE]
> 消费上限与有效期在**四种协议前端**（Anthropic / OpenAI / OpenAI-Responses / Gemini）上统一计量并拦截：无论调用方走哪个端点，用量都记在同一个 key 上，超限或过期即被拒绝。
...
重启与停机都走优雅退出：先停止接收新请求，再给在途请求一个**有上限的排空窗口（8 秒）**，到点即进入收尾流程。因此最后一次用量统计一定会落盘，不会被个别长连接拖住而丢账。
