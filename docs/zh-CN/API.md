...
> **v0.16.0:管理端两处行为变化**
> - **改优先级立即生效**。此前粘滞选号档下改了优先级不触发重新选号,等于没改。
> - **全池被判停后会自愈**。上游抖一阵把所有账号判停时,池子不再就此彻底不可用到有人重启;
>   自愈**不会**唤醒被永久禁用、被封禁、或配额尚未到恢复时刻的账号。
> **v0.15.0:两处行为变化**
...
> **v0.10.0 起,被判不可用的账号一律不再自行回池。** 此前只有 `banned` 是这样;额度耗尽(402)
> 走 30 分钟冷却、连续两次无明确失效信号的 401/403 走 5 分钟冷却,到点都会自动回到轮转里。
> 后果是一个**已被上游封停**的账号每 5 分钟被重新拿去打一次、永不停止,一个额度耗尽的账号
> 每 30 分钟去撞一次必然失败的墙(一天 48 次)。现在这两类与 `banned` 一样停止使用。
> 与 `banned` 的区别在于**持久性**:它们只置内存态,重启或「重置」即复活,不把上游一次
> 权限抖动变成永久损失;`banned` 则随 `statusReason` 落盘。
> 429 不在此列——它已改判为瞬时限流(退避重试,不罚账号)。
