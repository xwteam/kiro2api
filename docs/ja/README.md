...
| 日付 | 更新内容 |
|------|----------|
| 2026-08-19 | v0.17.2 - 📄 **ドキュメントのみの修正。コードは v0.17.1 と完全に同一で、再デプロイは不要です**。①多言語 README の更新履歴のうち 9 行が誤った言語でした(繁体字に簡体字、日本語と韓国語に英語)。2 種類の文面を 6 ファイルに流用し、言語ごとに書いていなかったのが原因です。現在は全 60 行を 1 行ずつ確認済みです。②5 つの API.md にバージョン契約が欠けていました —— v0.15.0 は日本語・韓国語・繁体字の 3 つに存在せず、しかもその版はサーバー側組み込み検索が実際に繋がった版(`server_tool_use` / `web_search_tool_result` を追加)で、この 3 言語の読者には全く見えていませんでした。v0.16.0 は 5 つすべてで欠落。現在は揃っています。③v0.17 の契約文を追加し、古いバージョン番号の残りを一掃しました |
